    ...
    methods: {
        async runWebGL() {
            ...
            topVertexColor.onchange = () => {
                ...
                vertices[3] = newColor[0];
                vertices[4] = newColor[1];
                vertices[5] = newColor[2];
                gl.bufferData(gl.ARRAY_BUFFER, vertices, gl.DYNAMIC_DRAW);
            }
            bottomLeftVertexColor.onchange = () => {
                let newColor = this.hexColorToGLRGB(bottomLeftVertexColor.value);
                vertices[9] = newColor[0];
                vertices[10] = newColor[1];
                vertices[11] = newColor[2];
                gl.bufferData(gl.ARRAY_BUFFER, vertices, gl.DYNAMIC_DRAW);
            }
            bottomRightVertexColor.onchange = () => {
                let newColor = this.hexColorToGLRGB(bottomRightVertexColor.value);
                vertices[15] = newColor[0];
                vertices[16] = newColor[1];
                vertices[17] = newColor[2];
                gl.bufferData(gl.ARRAY_BUFFER, vertices, gl.DYNAMIC_DRAW);
                console.log('change')
            }
...
fetch(`http://localhost:8000/shader?id=${"basicVertex.glsl"}`).then(res => res.json()).then(data => {app.webGLVShader = data.shader;});
